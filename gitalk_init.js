...
// 根据自己的情况进行配置
// 如果放到scripts文件夹，会自动执行，但是由于执行先后顺序的问题，会报错，所以不要放进去
// 移动此文件时，需要更改代码中两处相对路径
const config = {
    // 注意，next8以后的gitalk 对id字段是自带md5加密的，本js做了处理,见这条语句
    // let label = crypto.createHash('md5').update(pathLabel,'utf-8').digest('hex');
    username: "YJ2CS", // GitHub 用户名
    token: "0990c7f3e95760a95957fa6b19a8bec57f0f24ee",  // GitHub Token
    repo: "legendary-octo-pancake",  // 存放 issues的git仓库
    // sitemap.xml的路径，commit.js放置在根目录下，无需修改，其他情况自行处理
    sitemapUrl: path.resolve(__dirname, "./public/sitemap.xml"),
    kind: "Gitalk",  // "Gitalk" or "Gitment"，
    baseMd5Url: "" //此项按需设置，根据你的id中如何填写来设置
    // 注意，域名url 是你在./_config.yml中的url
};
        ...
        if (notInitIssueLinks.length > 0) {
            ...
            setTimeout(async ()=>{
                let initRet = await notInitIssueLinks.map(async (item) => {
                    ...
                    //path指代无域名前缀的url,如 /hello/index.html ,这是id的默认未加密值，下面将对其实行md5加密，以符合我们更改过后的id值
                    pathLabel = crypto.createHash('md5').update(config.baseMd5Url + pathLabel,'utf-8').digest('hex');
                    // pathLabel = md5(config.baseUrl + pathLabel);//中文过长所以要md5
                    let body = `${item}<br><br>${websiteConfig.description}`;
                    let form = JSON.stringify({ "body": body, "labels": [config.kind, pathLabel], "title": title });
